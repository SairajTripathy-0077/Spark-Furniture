          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleProductSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Sketch Image Type Fallback */}
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-[#31170E]/70">Fallback Sketch Type</label>
                    <select
                      value={imageType}
                      onChange={(e) => setImageType(e.target.value)}
                      className="w-full mt-2 rounded-xl border border-neutral-200 bg-white py-2.5 px-4 text-xs font-bold uppercase tracking-wider text-[#31170E] focus:border-[#31170E] focus:outline-none focus:ring-1 focus:ring-[#31170E]/20 cursor-pointer transition-all duration-300"
                    >
                      <option value="chair">Chair Sketch</option>
                      <option value="table">Table Sketch</option>
                      <option value="sofa">Sofa Sketch</option>
                      <option value="lamp">Lamp Sketch</option>
                      <option value="storage">Storage Sketch</option>
                    </select>
                  </div>
                  
                  {/* Add Colors */}
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-[#31170E]/70">Add Product Colors</label>
                    <div className="flex mt-2 gap-2">
                      <input
                        ...
                        className="flex-1 rounded-xl border border-neutral-200 bg-white py-2 px-3 text-[11px] font-semibold text-[#31170E] focus:border-[#31170E] focus:outline-none"
                      />
                      <input
                        type="color"
                        value={colorHex}
                        onChange={(e) => setColorHex(e.target.value)}
                        className="w-10 h-8 rounded-lg cursor-pointer border border-neutral-200 p-0"
                      />
                      <button
                        type="button"
                        onClick={handleAddColor}
                        className="rounded-xl bg-[#31170E] text-[#fdf9f4] px-3 py-2 text-[10px] font-bold uppercase tracking-wider hover:bg-[#6b4335] active:scale-95 cursor-pointer"
                      >
                        Add
                      </button>
                    </div>
                ...
                {colors.length > 0 && (
                  <div className="rounded-xl border border-neutral-200 bg-white p-3 space-y-2">
                    <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest block">Configured Colors</span>
                    <div className="flex flex-wrap gap-1.5">
                      {colors.map((c, i) => (
                        <span key={i} className="inline-flex items-center gap-1.5 rounded-full border border-neutral-200 bg-neutral-50 pl-2 pr-1 py-1 text-xs">
                          <span className="h-3.5 w-3.5 rounded-full border border-black/10 shadow-sm" style={{ backgroundColor: c.hex }} />
                          <span className="font-semibold text-neutral-700">{c.name}</span>
                          <button
                            type="button"
                            onClick={() => handleRemoveColor(i)}
                            className="rounded-full p-0.5 hover:bg-neutral-200 text-neutral-400 hover:text-black cursor-pointer"
                          >
                            <X size={10} />
                          </button>
                        </span>
